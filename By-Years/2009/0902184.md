...
lengthening growing season. This work will require intense mechanistic research
focusing on transitions and transformations that occur over only a few weeks at
most, but which have profound impacts on the tundra ecosystem. Researchers will
scale this mechanistic work to the intermediate spatial scale by conducting
transect measurements along a latitudinal transect to validate that patterns
that occur locally are robust. They will scale to the whole Arctic system by
integrating these mechanisms, and importantly, the N-effects on decomposition,
into the MEL model that is designed to explore multiple limiting resource
effects on ecosystem function. As an integrated package, this research will
explore how the changing seasonal pattern that drives the crash in N
availability in tundra soils will alter overall tundra C-cycling and its role as
a source or sink of C and through this its role in the global climate system.
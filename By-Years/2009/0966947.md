...
equations.&lt;br/&gt;&lt;br/&gt;This research is based on the framework in which
a viscous incompressible fluid can be explicitly represented as the average of
an inviscid fluid plus Brownian motion. A study of several aspects of fluid
dynamics in this framework is proposed. One important application is the
implementation of a numerical method to compute fluid flows in turbulent
settings (for instance air flow around an airplane wing, or in a jet engine). A
related application is to use this method to find a "turbulence model" for fluid
flows: namely, the velocity field of the fluid is expressed as a slowly varying
"mean field" (which is easy and inexpensive to compute), plus a rapid
fluctuating "noisy" part, which is modeled using stochastic methods. Another
application is to study boundary layer effects on the interior flow in the
inviscid limit. A study of burning and flame propagation is also
proposed.&lt;br/&gt;